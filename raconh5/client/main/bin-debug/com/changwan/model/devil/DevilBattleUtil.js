 ...
 * create 2018-04-17
*/
var DevilBattleUtil = (function () {
    function DevilBattleUtil() {
    }
    DevilBattleUtil.startWalkToCenter = function () {
    ...
    DevilBattleUtil.walkComplete = function () {
        this._tempTime = egret.setInterval(this.playOneAction, this, this.STEP_TIME);
        this._step = 2;
        var selfPos = Manager.model.getDevil().getSelfTargetPos();
        this._selfInfo.updatePostion(selfPos.x, selfPos.y);
        var enemyPos = Manager.model.getDevil().getEnemyTargetPos();
        this._enemyInfo.updatePostion(enemyPos.x, enemyPos.y);
        this.playOneAction();
    };
    DevilBattleUtil.playOneAction = function () {
        if (!Manager.model.getDevil().isGrabing)
            return;
        this.battleUpdate(true);
        this.battleUpdate(false);
        this._step++;
    };
    DevilBattleUtil.battleUpdate = function (isSelf) {
        if (!Manager.model.getDevil().isGrabing)
            return;
        ...
        if (attacked.attrInfo.hp <= 0)
            Manager.model.getDevil().showResultView();
    };
    DevilBattleUtil.getSkillCVO = function (selfAttack, skillFlag) {
    ...
    DevilBattleUtil.getHurtValue = function (selfAttack, hurtPercent) {
        if (hurtPercent == 0)
            return 0;
        var result;
        if (selfAttack) {
    ...
    DevilBattleUtil.readyData = function () {
        this._step = 1;
    ...
    DevilBattleUtil.clearData = function () {
        this._step = 1;
        this._petSkill = null;
    ...
    DevilBattleUtil.STEP_TIME = 500;
    return DevilBattleUtil;
}());
__reflect(DevilBattleUtil.prototype, "DevilBattleUtil");
//# sourceMappingURL=DevilBattleUtil.js.map