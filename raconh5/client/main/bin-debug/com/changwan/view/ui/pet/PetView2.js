...
*/
var PetView2 = (function (_super) {
    __extends(PetView2, _super);
    function PetView2() {
        var _this = _super.call(this) || this;
        _this.DRAW_PAGE = "drawPage";
        _this.DRAW_STRIP = "drawStrip";
        _this.DRAW_ANI = "drawAni";
        _this.touchChildren = true;
        _this._isUpgradeing = false;
        _this.start();
        _this.addEvent();
        return _this;
    }
    ...
    PetView2.prototype.createContainer1Img = function (y) {
        var result = BitmapRes.create("common_back3_png", 0, y, 227, 37);
        result.scale9Grid = new egret.Rectangle(8, 4, 49, 29);
        this._container1.addChild(result);
        return result;
    };
    PetView2.prototype.addEvent = function () {
    ...
    PetView2.prototype.removeEvent = function () {
        for (var i = 0; i < 4; i++) {
    ...
    PetView2.prototype.disposeSelf = function () {
        ...
        if (Manager.render.contains(this.drawLoss, this))
            Manager.render.remove(this.drawLoss, this);
        if (Manager.model.getGuide().curID == GuideID.PET_UPGRADE)
            Manager.control.getTask().hideGuide();
        if (Manager.render.contains(this.delayUpgradePet, this))
            Manager.render.remove(this.delayUpgradePet, this);
    ...
    PetView2.prototype.onTouchSkillHandler = function (e) {
        var grid = e.currentTarget;
        if (!grid)
            return;
        Manager.view.show(47 /* PetSkillView */, grid.cvo);
    };
    PetView2.prototype.prePage = function () {
        if (this._curPage <= 1)
            return;
        this._curPage--;
        this.invalidate(this.DRAW_PAGE);
    };
    PetView2.prototype.nextPage = function () {
        if (this._curPage >= this._totalPage)
            return;
        this._curPage++;
        this.invalidate(this.DRAW_PAGE);
    };
    PetView2.prototype.upgradeFun = function (e) {
        if (e != null && Manager.model.getGuide().curID == GuideID.PET_UPGRADE)
            return;
        if (this._isUpgradeing)
            return;
        var petCvo = PetCVO.getCVO(this._petModel.pinjie, this._petModel.star);
        if (petCvo && !petCvo.loss.isEnough()) {
            var shopCvo = ShopCVO.getbaseIdCvo(petCvo.loss.baseId);
    ...
    PetView2.prototype.drawStar = function () {
        if (!this._curCvo)
            return;
        this._starView.update(this._curCvo.star);
    };
    /**添加延迟，等进度条播放完毕再进阶 */
    PetView2.prototype.delayUpgradePet = function () {
        if (!this._isUpgradeing)
            return;
    ...
    PetView2.prototype.drawAttrAndFight = function () {
        if (!this._curCvo)
            return;
        var str = "";
        var info;
        var infos = this._petModel.allAttrVO.attrInfos;
    ...
    PetView2.prototype.drawAni = function () {
        if (!this._curCvo)
            return;
        if (this._petAni) {
            Manager.pool.push(this._petAni);
    ...
    PetView2.prototype.drawPage = function () {
    ...
    PetView2.prototype.drawLoss = function () {
        if (!this._curCvo)
            return;
        var bagCount = Manager.model.getItems().getCountItemById(this._curCvo.loss.baseId);
        var itemName = ItemsCVO.getCvo(this._curCvo.loss.baseId).name;
        var color = bagCount >= this._curCvo.loss.num ? Color.WHITE_STR : Color.RED_STR;
        var msg = StringUtils.setParam("{0}：{1}/{2}", itemName, HtmlUtil.addColorTag("" + bagCount, color), this._curCvo.loss.num);
        ...
        for (var i = 0; i < 4; i++) {
            if (this._grids[i].cvo) {
                var visible = this._petModel.checkOneSkillCanUp(this._grids[i].cvo.groupID);
                if (visible) {
                    if (this._skillReds[i].parent == null)
                        this._container2.addChild(this._skillReds[i]);
                }
                else {
                    if (this._skillReds[i].parent != null)
                        this._container2.removeChild(this._skillReds[i]);
                }
            }
        }
    };
    PetView2.prototype.drawAll = function () {
        ...
        this.drawAni();
        this.drawLoss();
        if (this._petModel.allAttrVO)
            this.drawAttrAndFight();
        else
            this._petModel.delayParseAllAttrVO();
        //引导(放在start会导致视图没添加到panel，获取parent为空)
        if (Manager.model.getGuide().curID == GuideID.PET_UPGRADE) {
            var pos = this._upgradeBtn.parent.localToGlobal(this._upgradeBtn.x, this._upgradeBtn.y);
            Manager.control.getTask().showGuide(pos, this._upgradeBtn.width >> 1, this._upgradeBtn.height >> 1, this.guideCB, this, false);
        }
    };
    PetView2.prototype.draw = function () {
        _super.prototype.draw.call(this);
        if (this.isInvalid(this.DRAW_STRIP))
            this.drawStrip();
        if (this.isInvalid(this.DRAW_PAGE))
            this.drawPage();
        if (this.isInvalid(this.DRAW_ANI, this.DRAW_PAGE))
            this.drawAni();
        if (this.isInvalid("drawAttrAndFight"))
            this.drawAttrAndFight();
    };
    PetView2.prototype.onTouchHandler = function (e) {
        switch (e.currentTarget) {
            ...
            case this._huanhuaBtn:
                ...
                break;
            case this._feedBtn:
                Manager.view.show(46 /* PetFeedView */);
                break;
    ...
    PetView2.prototype.onItemsUpdateHandler = function (e) {
        Manager.render.add(this.drawLoss, this, 300, 1, null, true);
    };
    PetView2.prototype.onPetUpdateHandler = function (e) {
        switch (e.type) {
    ...
    PetView2.prototype.guideCB = function () {
...
__reflect(PetView2.prototype, "PetView2");
//# sourceMappingURL=PetView2.js.map