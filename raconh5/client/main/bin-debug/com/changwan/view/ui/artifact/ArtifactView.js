...
var ArtifactView = (function (_super) {
    ...
    function ArtifactView() {
        ...
        _this.skinName = Manager.path.getSkinName("artifact", "ArtifactViewSkin");
        _this.touchChildren = true;
        return _this;
    }
    ArtifactView.prototype.configUI = function () {
        ...
        var arr = ArtifactCVO.getCvos();
        for (var i = arr.length - 1; i > -1; i--) {
            var cvo = arr[i];
            var loss = new GainLossVO(cvo.item);
            var item = this["_item" + i];
            if (item)
                item.setGainLossVO(loss);
        }
        this._item0.setBgHied(false);
        this._item1.setBgHied(false);
        this.starTween();
    };
    ArtifactView.prototype.starTween = function () {
        egret.Tween.get(this._group, { loop: false }).to({ y: 800 }, 600).call(this.star2Tween, this);
    };
    ArtifactView.prototype.star2Tween = function () {
        egret.Tween.get(this._group, { loop: false }).to({ y: 823 }, 600).call(this.starTween, this);
    };
    ArtifactView.prototype.initData = function () {
        _super.prototype.initData.call(this);
        this.drawData();
        //引导
        if (Manager.model.getGuide().curID == GuideID.ARTIFACT) {
            var pos = this._huntingOneBtn.parent.localToGlobal(this._huntingOneBtn.x, this._huntingOneBtn.y);
            Manager.control.getTask().showGuide(pos, this._huntingOneBtn.width >> 1, this._huntingOneBtn.height >> 1, this.guideCB, this, false);
        }
    };
    ArtifactView.prototype.addEvent = function () {
    ...
    ArtifactView.prototype.removeEvent = function () {
    ...
    ArtifactView.prototype.onTouceHandler = function (e) {
        var target = e.target;
        if (target == this._historyBoxImg) {
            Manager.view.show(111 /* ArtifactRewardView */);
            return;
        }
        if (target == this._takeTxt) {
            //寻宝记录
            Manager.view.show(112 /* ArtifactInfoListView */);
            return;
        }
    ...
    ArtifactView.prototype.clickHuntOne = function (e) {
        if (e != null && Manager.model.getGuide().curID == GuideID.ARTIFACT)
            return;
    ...
    ArtifactView.prototype.hidePnl = function () {
        Manager.view.hide(110 /* ArtifactPanel */);
    };
    ArtifactView.prototype.drawData = function () {
        this.drawStrip();
        this._artItem.setData(this._model.getItemList());
    };
    /**进度条 */
    ArtifactView.prototype.drawStrip = function () {
        var integ = this._model.getIntegral();
        var cvo = ArtifactIntegralCVO.getCurIntegralCvo();
    ...
    ArtifactView.prototype.drawLoss = function () {
        ...
        var tegralCvo = ArtifactIntegralCVO.getIsFristCvo();
        if (tegralCvo) {
    ...
    ArtifactView.prototype.guideCB = function () {
        var cvo = ArtifactLossCVO.getCvo(ArtifactType.SENDS_ONE_TYPE);
        var loss = new GainLossVO(cvo.loss);
        var isEnough = loss.isEnough();
        if (!isEnough) {
        ...
        if (isEnough)
            Manager.render.add(this.hidePnl, this, 2000, 1, null, true);
    };
    ArtifactView.prototype.dispose = function () {
        if (Manager.model.getGuide().curID == GuideID.ARTIFACT)
            Manager.control.getTask().hideGuide();
...
__reflect(ArtifactView.prototype, "ArtifactView");
