 ...
 * create 2018.1.18
 */
class CopySilverView extends UIComponent
{
	...
	{
		super();
		this.skinName = Manager.path.getSkinName("activity", "CopySilverViewSkin");
	...
	private onClickHandler(e:egret.TouchEvent):void
	{
		switch(e.currentTarget)
		{
			...
			case this._btnAdd:
                ...
                {
                    if(!CopyExpConfigCVO.add_silver_cound_need.isEnough(true)) return;
                    // 19	是否花费{0}购买1次挑战激活
                    let ok:CallBackInfo = Manager.pool.create(CallBackInfo, Manager.control.getCopy().buyCount, Manager.control.getCopy(), CopyConst.TYPE_SILVER);
    ...
    private updateCooling(e:CopyEvent):void
    {
    ...
    private onUpdateLevel(e?:GameObjectAttrEvent):void
    {
    ...
    {
        let cvo:CopySilverHardCVO = CopySilverHardCVO.getCurCVO();
        ...
        if(cvo.id < 10)HtmlUtil.setTextFlow(this._txtDesc, LangCVO.getContent("copy31", cvo.name));
	...
	{
		super.draw();
		if(this.isInvalid("drawCooling")) this.drawCooling();
		if(this.isInvalid("drawCount")) this.drawCount();
		if(this.isInvalid("drawItem")) this.drawItem();
		if(this.isInvalid("drawLevel")) this.drawLevel();
		if(this.isInvalid("drawCooling", "drawCount", "drawItem")) this.drawRedIcon();
	}
