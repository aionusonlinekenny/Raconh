 ...
 * 2017.12.14
 */
class ClubBuildView extends UIComponent
{
	private _ggBg:eui.Image;
	private _gonggao:Label;
	private _editBtn:Button;
	private _gfgBtn:eui.Group;
	private _clubHomeBtn:eui.Group;
	private _clubCareerBtn:eui.Group;
	private _btnActivity:eui.Group;
	private _btnClubBF:eui.Group;
	private _lunjiantaiBtn:eui.Group;
	private _clubBFRedIcon:eui.Image;
	private _masterName:Label;
	private _icon1:eui.Image;
	private _icon2:eui.Image;
	private _icon3:eui.Image;
	private _icon4:eui.Image;
	private _lunjiantaiRedIcon:eui.Image;

	private _back0:BitmapRemote;
	private _back1:BitmapRemote;
	private _gonggaoSp:egret.Shape;

	private _editNoticeView:ClubEditNoticeView;
	private _clubView:ClubView;
	private _careerView:ClubCareerView;

	private _thisParent:ClubPanel;

	private _clubInfo:ClubInfo;
	private _showViewId:number = -1;
	private _txtWidth:number = 0;

	public constructor()
	{
		super();
		this.skinName = Manager.path.getSkinName("club", "ClubBuildViewSkin");
    ...
    private updateArenaMaxAward(e:ArenaEvent=null):void
	{
	...
	private updateTower(e:GameObjectAttrEvent):void
	{
    ...
    private updateClubBFRewards(e:egret.Event = null):void
    {
	...
	private onClickHandler(e:egret.TouchEvent):void
	{
		...
		{
			case this._editBtn:
				this._editNoticeView = Manager.view.show(ViewID.ClubEditNoticeView, this._clubInfo.desc);
				break;
			case this._gfgBtn:
			...
			case this._clubHomeBtn:
				if(OpenCVO.isOpen(OpenConst.ID_CLUB_CENTER, true))
					this._clubView = Manager.view.show(ViewID.ClubView);
				break;
			case this._clubCareerBtn:
				if(OpenCVO.isOpen(OpenConst.ID_CLUB_CAREER, true))
					this._careerView = Manager.view.show(ViewID.ClubCareerView);
				break;
			...
			case this._btnClubBF:
				if(OpenCVO.isOpen(OpenConst.ID_CLUB_WAR, true))
					Manager.view.show(ViewID.ClubBFPanel);
				break;
			case this._lunjiantaiBtn:
				if(OpenCVO.isOpen(OpenConst.ID_ARENA_PK, true))
					Manager.view.show(ViewID.ClubLunjiantaiPanel);
				break;
		}
	}
	...
	public get clubHomeBtn():eui.Group
	{
	...
	private onUpdateClubNoticeHandler(e:ClubEvent):void
	{
	...
	private onClubIconShowHander(e:BaseEvent=null):void
	{
		let bool:boolean = Manager.model.getJingMai().checkCoin() || Manager.model.getJuyuan().checkCoin();
		this._icon3.visible = bool;
	}

	private guideCB():void
	{
		if(OpenCVO.isOpen(OpenConst.ID_CLUB_COPY, true)) Manager.link.link(LinkType.PANEL_CLUB_ARENA, 0);
	}
