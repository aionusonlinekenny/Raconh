 ...
 * create 2018-4-13
*/
class TitleView2 extends RenderSprite
{
    ...
    public constructor(showItemID:number)
    {
        ...
        this._showCVO = TitleCVO.getCVO(showItemID);
        this.start();
        this.addEvent();
    }
    ...
    {
        this.showAni();
        let arr:Array<number> = TitleCVO.TITLE_TYPE_ARR;
        let dataArr = [];
        let cvos:Array<TitleCVO>;
    ...
    private drawData():void
    {
        ...
        {
            if(this._cvo.isTimeLimited) Manager.render.add(this.countdown, this, 1000);
            else this._lossTxt.text = "剩余时间：永久";
            if(this._cvo.isUsing) this.setBtnLabel(3);
            else this.setBtnLabel(2);
        }
        else
        {
            let bagCount = Manager.model.getItems().getCountItemById(this._cvo.loss.baseId);
        ...
        let str = "";
        let info:AttrVoInfo;
        let infos = this._cvo.baseAttr.attrInfos;
    ...
    {
        if(!this._cvo) return;
        let now = Math.round(Manager.model.getLogin().serverTimeInfo.serverTime / 1000);
        let second = this._cvo.time - now;
        if(second <= 0)
        {
    ...
    private setBtnLabel(type:number):void
    {
        if(this._redIcon.parent) this._redIcon.parent.removeChild(this._redIcon);
        switch(type)
        {
            case 1:
                if(this._wearImg.parent) this._wearImg.parent.removeChild(this._wearImg);
                if(this._takeoffImg.parent) this._takeoffImg.parent.removeChild(this._takeoffImg);
                this.addChild(this._actImg);
                if(this._cvo && this._cvo.loss.isEnough())
                    this.addChild(this._redIcon);
                break;
            case 2:
                if(this._actImg.parent) this._actImg.parent.removeChild(this._actImg);
                if(this._takeoffImg.parent) this._takeoffImg.parent.removeChild(this._takeoffImg);
    ...
    private setBtnSkin(isAct:boolean):void
    {
        let upImg:eui.Image = <eui.Image>this._actBtn.getChildAt(0);
        if(upImg) upImg.source = isAct ? "common_btn1_1_png" : "common_btn2_1_png";
        let downImg:eui.SetProperty = <eui.SetProperty>this._actBtn.skin.states[1].overrides[0];
        if(downImg) downImg.value = isAct ? "common_btn1_2_png" : "common_btn2_2_png";
    }
        ...
        switch(e.currentTarget)
        {
            case this._actBtn:
                ...
                if(!this._cvo.isActived)
                {
                    ...
                    if(bagCount < this._cvo.loss.num)
                    {
                        let cvo:ItemsCVO = ItemsCVO.getCvo(this._cvo.loss.baseId);
                        Manager.view.show(ViewID.ItemsTips,cvo);
                    }
                    else
                    {
                        let goods = ItemsCVO.getCvo(this._cvo.loss.baseId);
                        Manager.control.getDress().actTitle(goods.type, goods.id);
                    }
                }
                else
                {
                    if(this._cvo.isUsing) Manager.control.getDress().takeoffTitle(this._cvo.templateID);
                    else Manager.control.getDress().wearTitle(this._cvo.templateID);
                } 
                break;
        }
    }

    private onSelectedHandler(e:TitleEvent):void
    {
    ...
    {
        super.disposeSelf();
        ObjectUtil.removes(this._bgImg, this._leftBgImg, this._rightBgImg, this._fightBg, this._fightImg, this._skillBack, this._titleImg, this._actBtn, 
            this._actImg, this._wearImg, this._takeoffImg, this._redIcon, this._attrTxt, this._lossTxt, this._fighting, this._roleModel, this._titleList);
