...
class CameraManager
{
    ...
    public setFocus(x:number,y:number)
    {
        ...
        {
            var centerX:number = (x <= this._centerRect.x) ? this._centerRect.x : (this._centerRect.x + this._centerRect.width);
            var centerY:number = (y <= this._centerRect.y) ? this._centerRect.y : (this._centerRect.y + this._centerRect.height);
            this._seeRect.x += (x - centerX);
            this._seeRect.y += (y - centerY);
            let max = Math.max(0,this._mapSeeRect.width - this._seeRect.width);
     ...
     * 更新游戏视角位置
     */
    public updateSeeRect(x:number, y:number):void
    {
        this._seeRect.x = x;
        this._seeRect.y = y;
    }
