...
class CVOManager
{
    ...
    public loadDataComplete(bytes:ArrayBuffer):void
    {
    ...
    private render():void
    {
        var a:number = egret.getTimer();
        var name:string;
        if(this._current < this._totalLen)
        {
            this._current ++;
            let fileName:string = this._data.readUTF();
            name = fileName;
            let len:number = this._data.readInt();
        ...
        if(egret.getTimer() -a > 50)egret.log(name,egret.getTimer() - a);
    }

    public complete(reduce:boolean = true):void
    {
        if(reduce)this._count --;
    ...
    private loadFileData(fileName:string,data:egret.ByteArray):void
    {
        data.position = 0;
        switch(fileName)
        {
